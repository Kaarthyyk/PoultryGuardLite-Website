...
import type { Batch, WeeklyEntry } from '@/types/models';
...
export default function HomePage() {
  ...
  // Fetch all batches across all farms
  useEffect(() => {
    let isMounted = true;
  ...
  if (loadingFarms || loadingScans || loadingBatches || loadingEntries) {
  ...
  const stats = [
    ...
    { label: 'Active Batches', value: activeBatchesCount, icon: FileText },
    { label: 'Total Birds', value: totalBirds.toLocaleString(), icon: Users },
    { label: 'Mortality %', value: `${mortalityPercent}%`, icon: Activity },
    { label: 'Feed (kg)', value: totalFeed.toLocaleString(), icon: Activity },
    { label: 'AI Scans', value: scans?.length ?? 0, icon: Activity },
  ];
